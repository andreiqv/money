...
def conv_network_1(x_image, output_size=2):
	...
	p1 = convPoolLayer(x_image, kernel=(5,5), pool_size=2, num_in=3, num_out=16, 
		func=tf.nn.relu, name='1') # 32 x 32 
	p2 = convPoolLayer(p1, kernel=(5,5), pool_size=2, num_in=16, num_out=32, 
		func=tf.nn.relu, name='2')  # 16 x 16
	p3 = convPoolLayer(p2, kernel=(4,4), pool_size=2, num_in=32, num_out=64, 
		func=tf.nn.relu, name='3')   # 8 x 8 
	p4 = convPoolLayer(p3, kernel=(3,3), pool_size=2, num_in=64, num_out=128, 
		func=tf.nn.relu, name='4')   # 4 x 4 

	# fully-connected layers
	fullconn_input_size = 4*4*128
	...
	f3 = fullyConnectedLayer(drop2, input_size=1024, num_neurons=output_size, 
